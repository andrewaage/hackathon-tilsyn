...
- Øvrige ord i bedriftsnavnet - det må vel være mer enn bare "kebab" som skiller seg ut? Er det her mest effektivt å resonnere seg frem til ord man skal lete etter, etter bruke automatiske metoder? 
...
```{r}
rec <- recipe(total_karakter ~ er_kebab + tilsynsbesoektype + poststed, 
              data = train) %>% 
...
Som valideringsmetric scores dere på macro-averaged MAE, som er et måltall som egner seg for ordinale utfall når utfallet er imbalanced, se mer:
...
```{r}
# TODO: Erstatt med faktisk scoredatasett
score_data <- df %>% slice_sample(n = 100)

score_data <- score_data %>% 
  mutate(final_prediction = predict(best_model, new_data = .)$.pred)
```
...
output <- score_data %>% 
  select(tilsynsobjektid, dato, final_prediction) %>% 
  write_csv2(str_glue("output/{teamnavn}_score.csv2"))
```
...
# OBS: For optimal tuning bør du bruke macro-averaged mae her
val_metrics <- yardstick::metric_set(mae, rmse, rsq)
